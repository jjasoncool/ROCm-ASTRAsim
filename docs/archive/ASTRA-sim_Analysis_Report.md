...
* [摘要](#摘要)
* [Alpha 定義與用途（先讀要點）](#alpha-定義與用途先讀要點)
* [問題描述](#問題描述)

  * [原始校準結果](#原始校準結果)
* [整合診斷與校準建議（摘要）](#整合診斷與校準建議摘要)
* [校準教學：如何以最小成本得到可靠的 alpha](#校準教學如何以最小成本得到可靠的-alpha)

  * [A. 前置條件（你已具備）](#a-前置條件你已具備)
  * [B. 最小化步驟（可在容器內執行）](#b-最小化步驟可在容器內執行)
  * [C. 自動化範例（ wrapper ）](#c-自動化範例-wrapper-)
  * [D. 若要套用 comm-specific alpha 到 pipeline](#d-若要套用-comm-specific-alpha-到-pipeline)
  * [E. Acceptance criteria（建議）](#e-acceptance-criteria建議)
* [快速重點 (TL;DR)](#快速重點-tldr)
* [Alpha（簡明定義）](#alpha簡明定義)
* [為什麼要取得 alpha？](#為什麼要取得-alpha)
* [校準方法（精要）](#校準方法精要)
* [alpha 的影響（一眼看懂）](#alpha-的影響一眼看懂)
* [關鍵問題指標](#關鍵問題指標)
* [實驗過程與發現](#實驗過程與發現)

  * [1. 檢查 ET 檔案內容](#1-檢查-et-檔案內容)
  * [2. PyTorch Traces 分析](#2-pytorch-traces-分析)
  * [3. ASTRA-sim 輸出分析](#3-astra-sim-輸出分析)
  * [4. 簡化測試驗證](#4-簡化測試驗證)
* [數字分析與結論](#數字分析與結論)
...
* [解決方案建議](#解決方案建議)
  ...
  * [長期解決方案](#長期解決方案)
* [結論](#結論)
* [新手指南：三個關鍵問題解答](#新手指南三個關鍵問題解答)

  * [問題 1：我的 trace 檔案有記錄這些資訊嗎？](#問題-1我的-trace-檔案有記錄這些資訊嗎)
  * [問題 2：ET 檔案有沒有轉出這些資訊？](#問題-2et-檔案有沒有轉出這些資訊)
  * [問題 3：ASTRA-sim 是否有 attribute 可以帶入這些資訊？](#問題-3astra-sim-是否有-attribute-可以帶入這些資訊)
* [簡單的診斷步驟](#簡單的診斷步驟)
* [實際驗證實驗](#實際驗證實驗)
* [總結給新手](#總結給新手)
* [補充分析：工作負載執行失敗問題調查](#補充分析工作負載執行失敗問題調查)
...
* [技術棧參考](#技術棧參考)
  ...
  * [驗證工具](#驗證工具)
  * [可用測試數據](#可用測試數據)
* [已實施的修正與更正說明（本報告更新）](#已實施的修正與更正說明本報告更新)
* [🔑 最新重大發現：NS-3 模擬卡死問題 - 已完全解決](#-最新重大發現ns-3-模擬卡死問題---已完全解決)
...
## 最新實驗結果與結論 (2025-12-09)

根據 `runs/calibration_all.csv` 中記錄的最新實驗數據，我們得出以下關鍵結論，這些結論驗證並擴充了本報告先前的分析。

### 代表性校準結果摘要

| 模型 (`tag`) | 總體 Alpha (`alpha_us`) | 通訊 Alpha (`alpha_comm_us`) | GPU Alpha (`alpha_gpu_us`) | 真實步驟時間 (`real_t_step_ms`) | 真實通訊時間 (`real_t_net_comm_ms`) | 通訊校準誤差 (`rel_err_comm_comm`) |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| (未標記) | 1.263901 | 1.234384 | 0.178782 | 137,996 | 321 | **0.000000** |
| (未標記) | 79.831682 | 109.226190| 0.426317 | 2,306,896| 3,552 | **0.000000** |
| `cifar10` | 0.655680 | 0.000000 | 0.215365 | 129,311 | 0 | (N/A) |

### 核心發現

1.  **`alpha` 值依賴於工作負載**:
    *   不同模型（`cifar10` vs. 未標記模型）展現出截然不同的 `alpha` 值。例如，`cifar10` 的 `alpha_us` 約為 `0.66`，而其他模型的 `alpha_us` 則在 `1.26` 到 `79.8` 之間波動。
    *   這強烈證明了**為不同工作負載進行獨立校準的必要性**，不存在一個適用於所有模型的通用 `alpha` 值。

2.  **組件特定 Alpha (`alpha_comm_us`) 的有效性**:
    *   在所有（非 `cifar10`）的最新實驗中，`rel_err_comm_comm`（使用通訊特定 alpha 計算的通訊時間誤差）均為 `0.000000`。
    *   這證實了本報告先前的核心論點：**使用 `alpha_comm_us` 可以完美地將模擬通訊 cycles 映射到真實通訊時間**，解決了 `rel_err_comm` 過高的問題。

3.  **系統感知校準對 `cifar10` 的影響**:
    *   `cifar10` 是一個 System-Bound 的模型，其 trace 是透過**系統感知校準** (`--force-avg-kernel-ns`) 生成的。
    *   在此模式下，通訊時間被攤提到計算節點中，因此 `real_t_net_comm_ms` 為 `0`，`alpha_comm_us` 也為 `0`。這是一個預期行為，表明模擬時間的準確性主要由 `alpha_us` 和 `alpha_gpu_us` 決定。

### 總結

最新的實驗數據再次確認，為了獲得準確的模擬結果，必須採用**針對特定工作負載和特定組件（通訊、計算）的 `alpha` 校準因子**。本報告後續章節所闡述的 `alpha` 定義、問題分析與校準方法論，已在這些最新的具體實踐中得到驗證。
...
* 為什麼要取得 alpha？
  ...
  * 取得合適的 alpha 可用於：
    ...
    * 校準模擬（將 sim cycles 轉為 ms）
    * 區分 comm 與 compute 的 mismatch
    * 在做擴展模擬時保持可比較性。
...
* 觀察到的 `rel_err_comm` 在多次模擬中呈恆定（範例值 0.355238），原因在於 `sim_cycles_comm` 與 `real_t_comm_ms` 在這些 run 中皆未改變，因此用相同 alpha 計算會得到相同誤差。
...
# run_calib.sh (示意)
for i in 1 2 3; do
  torchrun --standalone --nproc_per_node=2 ./src/train_rocm_pytorch.py --epochs 1 --profile-epoch 1 --trace-steps 1
  python ./src/conver_to_chakra_et.py --base-dir /workspace/data/chakra
...
* 在 `run_ns3.py` 新增 `--apply-comm-alpha`：

  * 若存在 `alpha_comm_us`，則在報表中使用 `sim_t_comm_ms_comm` 取代原本 `sim_t_comm_ms`（或同時輸出兩者以便比較）。
...
* calibration DB (`runs/calibration_all.csv`) 必須包含：
  `alpha_us, alpha_comm_us, alpha_gpu_us, sim_cycles_*, real_t_*, rel_err_*, run_dir, timestamp, trace_epoch、sample_count、IQR、container_image、script_commit、accepted_flag、notes`。
...
* **問題**：ASTRA-sim 在多次測試中對「通訊時間」的估算嚴重低估（`rel_err_comm` 常見 > 30%），但步驟總時間 (step) 看起來能被校準為一致。
...
* **原始工作負載**：ET 中 99 億週期 → ASTRA-sim 只用 64 週期
* **簡化測試**：ET 中 100 萬週期 → ASTRA-sim 只用 1 週期
* **縮放比例**：326:1 的巨大差異
...
1. **ASTRA-sim 未能正確讀取 / 使用 ET 檔案中的計算週期資訊**
2. **只模擬通訊部分，導致通訊時間被嚴重低估（89.56%）**
3. **這不是單純「重疊建模」的問題，而是 compute 時間在模擬中幾乎不存在**

這解釋了為什麼通訊時間誤差如此巨大——ASTRA-sim 實際上只用一個固定的最小值來代替所有的計算操作，從而導致了整個模擬結果的不準確性。
...
**答案：有的！你的 PyTorch trace 檔案記錄得很完整。**
...
1. **你的資料沒問題**：trace 和 ET 檔案都有完整的時間資訊。
2. **轉換沒問題**：從 trace 到 ET 的轉換是成功的。
3. **問題在 ASTRA-sim**：它不認識目前使用的計算時間屬性名稱。
...
* 拋出 `"Node 0 in ctrl_dep graph, but not found in index, file might be corrupted"` 錯誤
...
1. **向 Chakra 項目報告 Bug**：
   ...
   * 項目：[https://github.com/mlcommons/chakra](https://github.com/mlcommons/chakra)
...
以下列出在本倉庫中已實際實作或修正的項目，並說明哪些先前報告中的陳述已經被修正或更新：
...
2. 更正或澄清的報告敘述
   ...
   * 先前報告中敘述「ET 檔案完全包含計算週期但 ASTRA-sim 忽略」的結論仍成立，但補充：

     * 已在 repository 中加入多種輔助工具（mapping 與 patch）以嘗試回填或轉譯屬性名稱，這些變更已實作並驗證可執行，但尚未在所有 workload 上完全恢復 `comm_size` 或計算時間的正確讀取。
   * 先前提及的部分數值（例如某些簡化測試的 cycle/時間）來自少數測試執行的 snapshot；已將分析腳本改為更穩健的 per-rank median 聚合方式以避免單次波動造成誤導，並在報告中註明這個變更。

3. 尚未找到或仍待確認的真因

   * 核心未解：為何 ASTRA-sim 在讀取 ET 時會把計算週期縮小至 1 或 64 cycles（根本原因仍然可能是 ET 屬性名稱的不匹配或 ASTRA-sim feeder 的解析 bug）。
   * 已針對可能的屬性名稱做過嘗試與臨時修補（monkey-patch），但無法保證所有工作負載都會被正確解析，故仍需上游修正或進一步 debug ASTRA-sim feeder。
...
* 已修正報告中的敘述，移除過於肯定的結論（例如："ASTRA-sim 絕對不支援計算時間"），改為更精準的陳述：

  > "在目前的環境與 ET 內容下，ASTRA-sim 未能正確讀取或使用 ET 中的計算週期"。
  > 這一點在文件中已更新。
...
* `--trace-steps 8`（預設值）產生龐大 PyTorch traces。
* 轉換後的 ET 檔案包含**數千個節點**和**複雜依賴關係**。
* ASTRA-sim 解析龐大 ET 檔案時**資源耗盡**，導致永久卡死。
...
torchrun --standalone --nproc_per_node=2 ./src/train_rocm_pytorch.py \
  ...
  --trace-steps 1  # 🔑 關鍵修正：從 8 降低到 1-4
```
...
* ✅ `trace-steps 1`：成功完成，耗時約 9 秒
* ❌ `trace-steps 8`：永遠卡死，無任何響應
* 📊 規模差異：節點數從 5000+ 降至 <3000，資源使用大幅降低
...
1. **修改所有腳本**：將 `--trace-steps` 預設值從 8 降至 1
2. **更新文檔**：注明大規模 trace 的風險
3. **添加檢查**：在 NS-3 運行前驗證 ET 檔案規模

#### 🔄 長期改善方案

1. **動態規模調整**：根據可用記憶體自動選擇 `trace-steps`。
2. **分層 trace**：支持小規模快速測試 + 大規模詳細分析。
3. **上游修復**：向 ASTRA-sim 報告記憶體處理問題。
...
* **之前**：所有用戶都會遇到 NS-3 卡死，完全無法使用。
* **現在**：通過參數調整，模擬能在約 9 秒內完成。
* **影響**：讓整個 ASTRA-sim 網路模擬器恢復正常運作能力。

👉 **立即採取行動**：
將你的 `--trace-steps` 參數從 8 降至 1–4，然後就能正常使用 NS-3 網路模擬了！
