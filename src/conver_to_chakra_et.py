...
- 輸入：../data/chakra/pytorch_traces/{host_rank_*.json, device_rank_*.json}
- 輸出：../data/chakra/workload_et/allreduce.{rank}.et  （每個 rank 一個 .et）

做法：
1) 從 host_rank_* 找 "ProfilerStep#k" 取得每個 step 的 [start_us, end_us]。
2) 從 device_rank_* 於該窗內累積 GPU compute 與 comm（用 rccl/nccl/allreduce 等關鍵字辨識）。
...
def extract_steps_from_host(host_trace: Dict) -> List[Tuple[float, float]]:
    """回傳 [(start_us, end_us), ...] 依 ProfilerStep#k。"""
    steps = []
    for e in host_trace.get("traceEvents", []):
...
def collect_gpu_events(device_trace: Dict, window: Tuple[float, float]) -> Tuple[float, float]:
    """在時間窗 [start_us, end_us] 內累計 (compute_us_sum, comm_us_sum)。"""
    start_us, end_us = window
    comp_us, comm_us = 0.0, 0.0
    for e in device_trace.get("traceEvents", []):
        if e.get("ph") != "X":
            continue
        ts_us = float(e.get("ts", 0.0))
        dur_us = float(e.get("dur", 0.0))
...
def get_world_size(host_trace: Dict) -> int:
    di = host_trace.get("distributedInfo", {}) or {}
...
def main():
    ...
    ap.add_argument("--traces-dir", default=None,
                    help="host/device trace 目錄（預設：腳本目錄/../data/chakra/pytorch_traces）")
    ...
    ap.add_argument("--override-bytes", type=int, default=None,
                    help="直接指定每 step 每 GPU 的 ALLREDUCE bytes（優先於估算）")
    ...
    host_files = sorted(traces_dir.glob("host_rank_*.json"))
    device_files = sorted(traces_dir.glob("device_rank_*.json"))
    if not host_files:
        raise FileNotFoundError(f"找不到 host_rank_*.json 於 {traces_dir}")
    if not device_files:
        raise FileNotFoundError(f"找不到 device_rank_*.json 於 {traces_dir}")

    def rank_from_name(p: Path) -> int:
        m = re.search(r"rank_(\d+)\.json$", p.name)
        if not m:
            raise ValueError(f"檔名無法解析 rank：{p.name}")
        return int(m.group(1))

    hosts = {rank_from_name(p): p for p in host_files}
    devs  = {rank_from_name(p): p for p in device_files}
    ranks = sorted(set(hosts.keys()) & set(devs.keys()))
    if not ranks:
        raise RuntimeError("host/device 檔名沒有共同的 rank 交集，請確認。")

    # world_size：取 rank 最小的 host 檔
    world_size = get_world_size(load_trace(hosts[ranks[0]]))
    ...
    for r in ranks:
        host_trace = load_trace(hosts[r])
        dev_trace  = load_trace(devs[r])

        steps = extract_steps_from_host(host_trace)
        if not steps:
            raise RuntimeError(f"Host trace 無 ProfilerStep 事件：{hosts[r]}")
        ...
        with out_path.open("wb") as et:
            ...
            node_id = 1
            for step_idx, (s_us, e_us) in enumerate(steps, 1):
                comp_us, comm_us = collect_gpu_events(dev_trace, (s_us, e_us))
